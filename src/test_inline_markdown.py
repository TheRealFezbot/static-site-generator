...
class TestInlineMarkdown(unittest.TestCase):
    ...
    def test_split_images_no_images(self):
        node = TextNode(
            "This is just a node without an image",
            TextType.TEXT,
        )
        new_nodes = split_nodes_image([node])
        self.assertListEqual(
            [
                TextNode("This is just a node without an image", TextType.TEXT)
            ],
            new_nodes
        )
    ...
    def test_split_links(self):
        node = TextNode("This is a text with a link [to boot dev](https://www.boot.dev) and [link](https://example.com)", TextType.TEXT)
    ...
    def test_split_links_no_link(self):
        node = TextNode(
            "This is just a node without a link",
            TextType.TEXT,
        )
        new_nodes = split_nodes_link([node])
        self.assertListEqual(
            [
                TextNode("This is just a node without a link", TextType.TEXT)
            ],
            new_nodes
        )
